...
`@v0.2.1` pins that release. Swap it for `@main` to always track the tip of
...
| `RESEARCH_POOL_SIZE` | `60` | Pool of candidate links, and what the title/snippet triage sees before any page is opened. A dead, blocked or empty link doesn't use up a budget slot: it yields to the next one in the pool. A narrow pool keeps the right page out of the triage |
